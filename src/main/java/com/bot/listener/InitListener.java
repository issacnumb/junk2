...
import com.bot.thread.CoreThread;
...
public class InitListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        log.info("springboot容器初始化完成,开始运行机器人服务");

        //初始化杠杆
        SetLevelBean ethSetLevelBean = new SetLevelBean();
        ethSetLevelBean.setLeverage(150);

        //初始化eth杠杆
        ethSetLevelBean.setSymbol(weexETHUSDT);
        ethSetLevelBean.setSide(1);
        weexService.setLevel(ethSetLevelBean);
        ethSetLevelBean.setSide(2);
        weexService.setLevel(ethSetLevelBean);

        //初始化btc杠杆
//        ethSetLevelBean.setSymbol(weexBTCUSDT);
//        ethSetLevelBean.setSide(1);
//        weexService.setLevel(ethSetLevelBean);
//        ethSetLevelBean.setSide(2);
//        weexService.setLevel(ethSetLevelBean);
        ...
        while (true) {
            ...
            // 全局遗漏异常捕捉
            try {
                CompletableFuture<String> future = coreThread.work();
                String string = future.join();// 等待线程执行完成并获取结果
...
//
////----------------------------------------------ETH过滤需要平仓的订单号-----------------------------------------------------
//                List<TpPackageBean> positionList = storage.getCurrentPosition();
...
//                }
                ...
                log.error("全局异常捕捉 : message : {}", e.getMessage());
                log.error("全局异常捕捉 : stack : {}", e.getStackTrace());
            }
        }
    }
}
